...
class Security
{
    ...
    public function maybe_hide_login_form()
    {
        ...
        if ( isset( $asenha_limit_login ) && $asenha_limit_login['within_lockout_period'] ) {
    ...
    public function obfuscate_string( $atts )
    {
        ...
        $link = $atts['link'];
        $class = $atts['class'];
        return '<div style="display:inline;unicode-bidi:bidi-override;direction:rtl;" class="' . esc_attr( $class ) . '">' . esc_html( $email_rev_parts[0] ) . '<span style="display:none;">obfsctd</span>&#64;' . esc_html( $email_rev_parts[1] ) . '</div>';
    }
